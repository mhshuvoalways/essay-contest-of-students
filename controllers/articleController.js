...
const updateArticle = (req, res) => {
  ...
  if (req.user.role === "admin") {
    Grade.find()
      .then((response) => {
        if (req.body.marks && req.body.link) {
          response.map((el) => {
            if (
              ...
              Article.findOne({ _id: id })
                .then(function (article) {
                  article.sharedLinks.push({ link: req.body.link });
                  Article.findOneAndUpdate(
                    { _id: id },
                    {
                      finalMarks: req.body.marks,
                      grade: el.gradeName,
                      sharedLinks: article.sharedLinks,
                    },
                    { new: true }
                  )
                    .populate("author")
...
//       }
//     } else if (req.body.link) {
//       const sharedLinks = { author: req.user._id, link: req.body.link };
//       Article.findOneAndUpdate(
//         { _id: id },
//         {
//           sharedLinks,
...
//       Article.findOneAndUpdate(
//         { _id: id },
//         { finalMarks: req.body.marks, grade: el.gradeName, sharedLinks },
//         { new: true }
//       )
//         .populate("author")
//         .then((response) => {
